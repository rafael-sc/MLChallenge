...
class ProductPagingSourceTest {
    ...
    @Test
    fun `load returns page when response is successful`() =
        runTest {
            ...
            coEvery { productApi.searchProducts(any(), any(), any()) } returns
                Response.success(
                    mockResponse,
                )
    ...
    @Test
    fun `load returns error when response is unsuccessful`() =
        runTest {
            val responseBody =
                mockk<ResponseBody> {
            ...
            coEvery { appLogger.error(any(), any(), any()) } just Runs
    ...
    @Test
    fun `load returns error when API throws exception`() =
        runTest {
            coEvery {
