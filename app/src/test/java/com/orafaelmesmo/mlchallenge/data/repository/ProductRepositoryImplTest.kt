...
class ProductRepositoryImplTest {
    ...
    @Test
    fun `getProductDetails should return success when API call is successful`() =
        runBlocking {
            // Arrange
            val productId = "MLA123456"
            val productDetailsResponse =
            ...
            coEvery { apiService.getProductDetails(productId) } returns
                Response.success(
                    productDetailsResponse,
                )
            coEvery { apiService.getProductDescription(productId) } returns
                Response.success(
    ...
    @Test
    fun `getProductDetails should return failure when API call fails`() =
        runBlocking {
            // Arrange
            val productId = "MLA123456"
            coEvery { apiService.getProductDetails(productId) } returns
    ...
    @Test
    fun `searchProducts should return flow of paging data`() =
        runBlocking {
            // Arrange
            val query = "iphone"
            val searchResponse =
            ...
            coEvery { apiService.searchProducts(query, any(), any()) } returns
                Response.success(
                    searchResponse,
                )
